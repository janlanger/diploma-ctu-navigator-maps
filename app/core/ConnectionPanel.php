	...
	protected function processQuery(array $query)
	{
		...
		if ($this->doExplains && isset($query[self::EXPLAIN])) {
			...
			$s .= "<br /><a href='#' class='nette-toggler' rel='#nette-Doctrine2Panel-row-$counter'>explain&nbsp;&#x25ba;</a>";
                ...
                if (isset($query[self::CALLER])) {
			
			$s .= "<br /><a href='#' class='nette-toggler' rel='#nette-Doctrine2PanelCall-row-$counter'>callstack&nbsp;&#x25ba;</a>";
		}
		...
		if ($this->doExplains && isset($query[self::EXPLAIN])) {
			...
			$s .= '</table>';
                ...
                $s.="<table id='nette-Doctrine2PanelCall-row-$counter' class='nette-collapsed'>";
                foreach($query[self::CALLER] as $caller) {
                    if(is_array($caller) && isset($caller['file'])) {
                        $s.="<tr><td><a href=editor://open?file=".$caller['file'].'&amp;line='.$caller['line'].'">'.  basename($caller['file']).':'.$caller['line'].'</a></td>';
                        $s.="<td>".(isset($caller['class'])?$caller['class'].$caller['type']:"").$caller['function'].'</td>';
                        $s.="</tr>";
                    }
                }
                $s.="</table>";
