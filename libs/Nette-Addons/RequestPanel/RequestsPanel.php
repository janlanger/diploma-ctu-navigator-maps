...
class RequestsPanel extends Object implements IBarPanel {
	...
	public function getPanel() {
		...
		if ($this->response instanceOf TextResponse ) {
			unset($session->logs);
			ob_start();
			require dirname(__FILE__) . '/bar.requests.panel.latte';
			return ob_get_clean();
		}
	}
	...
	public function onShutdown($presenter, $response) {
		...
		$entry['dumps']['HttpRequest'] = Debugger::dump($httpRequest, TRUE);
		$entry['dumps']['PresenterRequest'] = Debugger::dump($request, TRUE);
		$entry['dumps']['PresenterResponse'] = Debugger::dump($response, TRUE);
