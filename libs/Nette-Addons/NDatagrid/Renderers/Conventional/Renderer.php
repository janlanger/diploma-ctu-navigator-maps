...
class Conventional extends Nette\Object implements IRenderer
{
	...
	/** @var string */
	public $paginatorFormat = '%label% %input% z %count%';

	/** @var string */
	public $infoFormat = 'Zobrazuji záznamy %from% až %to% z %count% | Záznamů na stránku: %selectbox% | %reset%';
	...
	public function renderBody()
	{
		...
		if ($this->dataGrid->paginator->itemCount) {
			...
			$cell->add(Html::el('div')->setText($this->dataGrid->translate('Nebyly nalezeny žádné záznamy.')));
			$row->add($cell);
			$body->add($row);
		}
	...
	public function renderPaginator()
	{
            ...
            $vp=$this->dataGrid->getVisualPaginator();
		...
		foreach ($this->dataGrid->getColumns() as $column) {
			...
			if ($column->isOrderable()) {
				...
				$up->addClass($a ? 'active' : '')->href($column->getOrderLink('a'))
					->add(Html::el('span')->class('up'))->setTitle('Vzestupně');
				$down->addClass($d ? 'active' : '')->href($column->getOrderLink('d'))
					->add(Html::el('span')->class('down'))->setTitle('Sestupně');
